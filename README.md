...
GitHub Pagesで動作するPWAです。v5.3.1では、現在配信中の雀魂Web版をWorkerが解析し、現行WebSocketゲートウェイの`Lobby.fetchGameRecord`から牌譜Protobufを取得します。牌譜内容のデコードは次バージョンで実装予定です。

デプロイ済みWorker：`https://mahjong-paipu-proxy.mahjong-paihu.workers.dev`

## v5.3.1：認証Secretの安全な登録手順
...
11. 雀魂からログアウトした後、期限切れ、端末無効化、`OAUTH_REJECTED`表示時は、手順1〜7を繰り返してSecretを更新します。OAuthアクセストークンの有効期限は雀魂側が管理し、ログアウトやセッション失効で利用できなくなる場合があります。
...
- 値の種類: 現行WebSocketの`.lq.Lobby.oauth2Check`または`.lq.Lobby.oauth2Login`送信値から端末内で抽出した、アカウント種別とOAuthアクセストークン
- JSON必須フィールド: `type`（整数）、`accessToken`（空でない文字列）
...
- 再取得条件: `TOKEN_EXPIRED`、`OAUTH_TOKEN_INVALID`、`ACCOUNT_MISMATCH`、`OAUTH_REJECTED_UNKNOWN`が表示されたとき
...
v5.3.1では`SECRET_LOADING → SECRET_VALIDATION → OAUTH_REQUEST_BUILD → OAUTH_RPC → OAUTH_CHECK_RESPONSE → OAUTH_LOGIN_RESPONSE → SESSION_ESTABLISHED → FETCH_GAME_RECORD`を個別判定します。診断には`authStage`、`authState`、`oauthRpcName`、`oauthResponseReceived`、`sessionEstablished`、`fetchGameRecordReached`、`safeErrorCode`、`safeErrorMessage`、`nextAction`だけを保存します。
...
```json
{"ok":true,"service":"mahjong-paipu-proxy","version":"5.3.1"}
```
