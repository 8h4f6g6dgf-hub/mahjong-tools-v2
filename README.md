...
GitHub Pagesで動作するPWAです。v5.3.5では、`fetchGameRecord`を現在の共有ページを含むHARと端末内で比較し、完全一致したfield順・wire type・値の出所だけからWorkerリクエストを生成します。
...
3. JSONの必須フィールドは`flowVersion`（文字列）、`connectionType`（整数）、`clientVersionString`（文字列）、`providerType`（整数）、`prepareLoginToken`（文字列）、`fetchGameRecordProfile`（値を含まないHAR構造）です。
...
- JSON必須フィールド: `flowVersion`、`connectionType`、`clientVersionString`、`providerType`、`prepareLoginToken`、`fetchGameRecordProfile`
- `fetchGameRecordProfile`: RPC名、Envelope field構造、request field構造、各fieldの安全な値の出所だけを保存する。牌譜ID、Payload、生リクエストは保存しない
...
v5.3.5では`requestMatchScore`、`envelopeMatchScore`、`fieldMatchScore`、`messageMatchScore`だけを診断へ保存します。差分の値やPayloadは保存しません。4項目を検証できない場合、Workerは`fetchGameRecord`を送信しません。
...
```json
{"ok":true,"service":"mahjong-paipu-proxy","version":"5.3.5"}
```
...
まだ一致を確認できていないものは、v5.3.5公開後に取得する新しい共有ページHARと実際のWorker送信結果です。旧Secretには`fetchGameRecordProfile`がないため、手順4〜8を一度実施するまで`secretSchemaValid`はfalseになります。
