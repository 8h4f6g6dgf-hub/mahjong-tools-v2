...
GitHub Pagesで動作するPWAです。v5.3.7では、HAR登録ツールとWorkerのProfile Schemaを共通化し、静的検証成功後に同一WebSocket上の実行時検証へ進みます。

デプロイ済みWorker：`https://mahjong-paipu-proxy.mahjong-paihu.workers.dev`

## v5.3.7：認証SecretとHAR意味情報の安全な登録手順
...
v5.3.7では既存の4スコアを構造一致として扱い、`requestSemanticMatched`、`connectionContextMatched`、`requestIdSequenceMatched`、field 1/2の出所確認を別に判定します。値、差分、Payloadは診断へ保存しません。構造が100%でも意味が未確認ならWorkerは`fetchGameRecord`を送信しません。
...
- v5.3.6で全条件がtrueでも停止した原因は、総合条件が初期値`requestSemanticMatched=false`自身を参照していたためです。v5.3.7では個別条件だけから再計算します。
...
```json
{"ok":true,"service":"mahjong-paipu-proxy","version":"5.3.7"}
```
...
まだ一致を確認できていないものは、v5.3.7公開後に取得する新しい共有ページHARと実際のWorker送信結果です。旧Secretには`fetchGameRecordProfile`がないため、手順4〜8を一度実施するまで`secretSchemaValid`はfalseになります。
