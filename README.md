...
GitHub Pagesで動作するPWAです。v5.3.3では、現在の雀魂Web版で確認した共有URLから牌譜画面までのRPC順序をWorkerへ実装します。牌譜内容の完全デコードは次バージョンで実装予定です。

デプロイ済みWorker：`https://mahjong-paipu-proxy.mahjong-paihu.workers.dev`

## v5.3.3：認証Secretの安全な登録手順
...
v5.3.3では認証段階に加えて、値を含まない`rpcTimeline`、`rpcSequence`、`recordResolveReached`、`fetchGameRecordInputType`、`nextRpc`を保存します。
...
```json
{"ok":true,"service":"mahjong-paipu-proxy","version":"5.3.3"}
```
