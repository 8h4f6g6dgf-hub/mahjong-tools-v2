...
GitHub Pagesで動作するPWAです。v5.3.8では、HAR登録時に`prepareLogin`応答から`fetchGameRecord`送信までのSession Timelineを安全なメタデータとして抽出し、Workerが確認済み条件だけを再現します。

デプロイ済みWorker：`https://mahjong-paipu-proxy.mahjong-paihu.workers.dev`

## v5.3.8：認証SecretとHAR意味情報の安全な登録手順
...
v5.3.8では既存の4スコアを構造一致として扱い、`requestSemanticMatched`、`connectionContextMatched`、`requestIdSequenceMatched`、field 1/2の出所確認を別に判定します。値、差分、Payloadは診断へ保存しません。構造が100%でも意味が未確認ならWorkerは`fetchGameRecord`を送信しません。
...
- v5.3.6で全条件がtrueでも停止した原因は、総合条件が初期値`requestSemanticMatched=false`自身を参照していたためです。v5.3.8では個別条件だけから再計算します。

### v5.3.8のSession Timeline
...
```json
{"ok":true,"service":"mahjong-paipu-proxy","version":"5.3.8"}
```
...
まだ一致を確認できていないものは、v5.3.8公開後に取得する新しい共有ページHARと実際のWorker送信結果です。旧Secretには`fetchGameRecordProfile`がないため、手順4〜8を一度実施するまで`secretSchemaValid`はfalseになります。
